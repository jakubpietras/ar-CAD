...
namespace ar
{
	...
	ar::ToolPath PathGenerator::GenerateOutlineMill(MillingConfig config, ar::Entity outline, ar::Entity startPoint, ar::mat::Vec3 offsetDir)
	{
		...
		auto points = ar::GeneralUtils::GetPos(curve.Points);
		auto start = startPoint.GetComponent<TransformComponent>().Translation;

		size_t startIndex = 0;
		for (size_t i = 0; i < points.size(); ++i)
		{
			if (ar::mat::LengthSquared(points[i] - start) < 1e-6f)
			{
				startIndex = i;
				break;
			}
		...
		// 3. go through the curve until the end
		size_t count = points.size();
		for (size_t i = 0; i <= count; ++i)
		{
			size_t idx = (startIndex + i) % count;
			auto& p = points[idx];
			path.MoveTo({ p.x, p.y, m_BaseMargin });
		}
		
		// 4. move up and towards center
		path.MoveBy({ 0.f, 0.f, (config.StartPoint.z - m_BaseMargin) });
		path.MoveTo(config.StartPoint);
